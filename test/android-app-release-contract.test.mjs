...
test("Pi Droid app is conditional, fixed-identity, source-gated, and release signed", async () => {
  ...
  assert.match(build, /piDroidVersionCode"\)\.getOrElse\("9"\)/);
  assert.match(build, /piDroidVersionName"\)\.getOrElse\("0\.3\.0-internal\.9"\)/);
  ...
  assert.match(releaseNotes, /live e2e integration proof against real Pi Daemons/);
  assert.match(releaseNotes, /web dashboard UUID compatibility/);
  ...
  assert.match(releaseProperties, /^versionCode=9$/m);
  assert.match(releaseProperties, /^versionName=0\.3\.0-internal\.9$/m);
});
...
test("release script materializes secrets privately and verifies fixed identity before upload", async () => {
  ...
  assert.match(script, /--skip-emulator/);
  assert.match(script, /"emulatorEvidence": \$emulator_evidence/);
  assert.match(script, /upload-prepared reads emulator evidence state from the verified build receipt/);
  assert.match(script, /if \[\[ "\$upload_prepared" != 'true' && "\$emulator_evidence" == 'true' \]\]/);
  assert.match(script, /write_evidence_checksums/);
...
test("manual release workflow is isolated from ordinary CI and retains exact evidence", async () => {
  ...
  assert.match(workflow, /--skip-emulator/);
