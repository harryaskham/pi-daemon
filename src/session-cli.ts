...
async function readOwnerOnlyText(path: string, label: string, maxBytes: number): Promise<string> {
  ...
  if (info.isSymbolicLink() || !info.isFile() || (hasForeignPathOwner(info.uid, "owner-only", getuid?.())) || hasForbiddenExposure(info.mode, "private") || info.size < 1 || info.size > maxBytes) {
