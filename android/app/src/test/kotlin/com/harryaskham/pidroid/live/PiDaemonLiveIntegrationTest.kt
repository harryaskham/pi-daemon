...
class PiDaemonLiveIntegrationTest {
  ...
  @BeforeEach
  fun setUp() {
    val ownerOnly = PosixFilePermissions.asFileAttribute(setOf(
    ...
    Files.setPosixFilePermissions(tokenFile.toPath(), setOf(
      PosixFilePermission.OWNER_READ,
      PosixFilePermission.OWNER_WRITE
    ))
    ...
    val processBuilder = ProcessBuilder(
      ...
      "--ready-file", readyFile.absolutePath,
  ...
  @Test
  fun testLiveClientEndToEndSessionFlowAgainstRealDaemon() = runTest {
    withContext(Dispatchers.Default.limitedParallelism(1)) {
      val transport = OkHttpPiDaemonTransport(webSocketPingInterval = Duration.ofSeconds(5))
      var hostIdCounter = 0

      val credentialVault = HostCredentialVault(
        protector = object : CredentialProtector {
          override suspend fun protect(handle: CredentialHandle, bearer: CharArray) =
            ProtectedCredential.fromBytes(bearer.concatToString().toByteArray())

          override suspend fun reveal(handle: CredentialHandle, credential: ProtectedCredential) =
            credential.copyBytes().decodeToString().toCharArray()

          override suspend fun destroy(handle: CredentialHandle) = Unit
        },
        store = object : NoBackupCredentialStore {
          ...
          override suspend fun write(handle: CredentialHandle, credential: ProtectedCredential) {
            map[handle] = credential
          }
          override suspend fun read(handle: CredentialHandle) = map[handle]
          override suspend fun remove(handle: CredentialHandle) { map.remove(handle) }
        }
      )

      val hostRegistry = HostRegistry(
        store = object : HostRegistryStore {
      ...
      val defaultHostStore = object : DefaultHostStore {
        var current: HostId? = null
        override suspend fun read() = current
        override suspend fun write(hostId: HostId?) { current = hostId }
      }

      val dailyDriverStore = object : DailyDriverStore {
        ...
        override suspend fun writeSelectedInventory(hostId: HostId, inventoryId: String?) {
        ...
        override suspend fun writeActionBookmark(bookmark: LiveSessionActionBookmark?) { this.bookmark = bookmark }
        override suspend fun readInteractiveResume() = resume
        override suspend fun writeInteractiveResume(encoded: ByteArray?) { resume = encoded }
      }
      ...
      val readyState = withTimeout(10_000) {
      ...
      val connectedObserver = withTimeout(10_000) {
      ...
      val controllerState = withTimeout(10_000) {
        repository.interactiveState.filterIsInstance<LiveInteractiveAppState.Ready>().first {
      ...
      val promptResultState = withTimeout(10_000) {
        repository.interactiveState.filterIsInstance<LiveInteractiveAppState.Ready>().first {
      ...
      val releasedObserver = withTimeout(10_000) {
        repository.interactiveState.filterIsInstance<LiveInteractiveAppState.Ready>().first {
