...
- packaged content-hashed Dash SPA plus a loopback-only browser server wired into enabled embedded `serve` lifecycles, with atomic multi-listener startup/drain, strict CSP/static admission, digest-only web credential exchange, HMAC-signed revocable HttpOnly sessions, exact Host/Origin/CSRF policy, bounded WebSocket handoff, and atomic revisioned workspace/UI-settings persistence
